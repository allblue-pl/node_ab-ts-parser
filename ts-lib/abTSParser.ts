...
export class abTSParser_Class {
    ...
    static parseData(scriptFSPath: string, scriptPath: string, data: string, 
            exportDefines: Array<string>, errors: Array<string>) {
    ...
    static #getImportPath(rawImportPath: string): string {
    ...
    static #parseData_AddJSLibsExports(scriptFSPath: string, scriptPath: string, 
            sourceFile: SourceFile, data: string, exportDefines: Array<string>, 
    ...
    static #parseData_RemoveExports(data: string): string {
        return data.replace(/(^|\r\n)([ \t])*export([ \t])*(default([ \t])*)?(.+?)(;|\r\n)/gm, 
                "$1/* export$2$3$4*/$6$7");
    }

    static #parseData_ReplaceImports(sourceFile: SourceFile, data: string): string {
        ...
        for (let importDeclaration of importDeclarations) {
            ...
            for (let namedImport of namedImports) {
                ...
                this.#parseData_ReplaceImports_FindReferences(importDeclaration,
                        ...
                        (nameNode as Identifier).findReferences(), replaces);
            }
        }
    ...
    static #parseData_ReplaceImports_FindReferences(importDeclaration: ImportDeclaration,
